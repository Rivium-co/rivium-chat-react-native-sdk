...
// Types that would come from @rivium/react-native-chat
...
export interface RiviumChatClient {
  ...
  sendMessage(roomId: string, content: string, options?: { attachments?: Attachment[]; replyToId?: string }): Promise<Message>;
  ...
  onReadReceipt: (callback: (event: { roomId: string; userId: string; readAt: Date; lastReadAt?: string }) => void) => () => void;
}
