...
export class RealtimeService {
  ...
  async connect(): Promise<void> {
    ...
    this.client.on('error', (ctx) => {
      ...
      this.emit('connectionState', { state: 'error', error: ctx.error });
    });
  ...
  /** Publish typing indicator via Centrifugo (throttled to 2 seconds). */
  ...
  async getRoomPresence(roomId: string): Promise<Set<string>> {
    ...
    try {
      const result = await sub.presence() as any;
      const users = new Set<string>();
      for (const clientInfo of Object.values(result.clients) as any[]) {
        if (clientInfo.user) {
  ...
  private handleChatPublication(roomId: string, data: unknown): void {
    ...
    const eventData = (json.data ?? json) as Record<string, unknown>;
    ...
    switch (type) {
      ...
      case 'message_pinned':
        this.emit('messagePinChanged', {
          ...
          roomId,
          isPinned: true,
          pinnedBy: eventData.pinnedBy as string | undefined,
